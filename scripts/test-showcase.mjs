...
const pageCount = Math.ceil(SHOWCASE_CASES.length / 24)
const pages = Array.from({ length: pageCount }, (_, index) => getShowcasePage(SHOWCASE_CASES, String(index + 1)))
assert.deepEqual(pages.map((page) => page.items.length), Array.from({length:pageCount}, (_, i) => Math.min(24, SHOWCASE_CASES.length - i * 24)))
...
assert.equal(getShowcasePage(SHOWCASE_CASES, '999').page, pageCount)
