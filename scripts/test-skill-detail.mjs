...
const correctedMetadata = buildAgentReadableSkillMetadata({...skill, github_repo:'obra/superpowers',category:'Design'})
assert.equal(correctedMetadata.skill.category,'coding-agents')
assert.equal(correctedMetadata.install.ready,false, 'A category correction cannot make a source-unconfirmed skill installable')
