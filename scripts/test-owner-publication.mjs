...
const refreshSql = source('supabase/migrations/20260910164646_owner_publication_exact_source_refresh.sql')
assert.equal((refreshSql.match(/create or replace function/g) || []).length, 1)
assert.match(refreshSql, /security invoker/)
assert.match(refreshSql, /current_user <> 'service_role'/)
assert.match(refreshSql, /source_path = v_path and source_content_hash = v_hash/)
assert.match(refreshSql, /v_result is null and v_same_revision/)
assert.match(refreshSql, /jsonb_each\(p_skill - 'slug' - 'github_last_pushed_at'\)/)
