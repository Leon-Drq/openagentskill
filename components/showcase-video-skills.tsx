...
export function ShowcaseVideoSkills({ locale }: { locale: Locale }) {
  ...
  return <details ref={sectionRef} id="video-skills" className="mt-12 scroll-mt-24 rounded-lg border border-[#e4e0d8] p-5 sm:p-6">
    <summary className="min-h-11 cursor-pointer text-sm font-semibold text-[#006b4f]">{galleryCopy(locale, "Related video skills · 5 tools", "相关视频技能 · 5 个工具")}</summary>
    <p className="mt-3 max-w-3xl text-sm leading-relaxed text-[#6d675e]">{galleryCopy(locale, "Making a product demo or editing footage? Explore these tools and their requirements. Recommendations do not imply authorship of the examples above or platform testing; skills without a showcased output are not counted as examples.", "想制作产品演示或剪辑视频？查看以下技能的使用条件。它们是工具推荐，不是上方案例的作者归属，也不代表本站实测；没有对应成片的技能不计入作品数量。")}</p>
    <ul className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {SHOWCASE_VIDEO_SKILLS.map((skill) => <li key={skill.slug} className="min-w-0 rounded-lg border border-[#e4e0d8] bg-white/50 p-4">
        ...
        <div>
          ...
          <a href={skill.source} target="_blank" rel="noreferrer noopener" className="inline-flex min-h-11 items-center text-xs text-[#6d675e] underline underline-offset-4">{galleryCopy(locale, "View inspected source", "查看来源版本")}</a>
        </div>
      </li>)}
    </ul>
  </details>
