      ...
      <main>
        ...
        <section className="mx-auto max-w-6xl px-6 pb-20 pt-8" aria-label={galleryCopy(locale, "Browse examples", "浏览作品")}>
          ...
          {pagination.pageCount > 1 && <nav aria-label={galleryCopy(locale, "Gallery pagination", "作品分页")} className="mt-12 flex flex-wrap items-center justify-center gap-2">
            ...
            {Array.from({ length: pagination.pageCount }, (_, index) => index + 1).map((page) => <Link key={page} prefetch={false} href={pageHref(page)} aria-current={page === pagination.page ? 'page' : undefined} aria-label={galleryCopy(locale, 'Page {page}', '第 {page} 页', { page })} className={`inline-flex h-11 min-w-11 items-center justify-center rounded-md border px-3 text-sm ${page === pagination.page ? 'border-[#006b4f] bg-[#006b4f] text-white' : 'border-[#e4e0d8] text-[#6d675e] hover:border-[#006b4f]'}`}>{page}</Link>)}
