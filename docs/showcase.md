...
新增足球榜单短片和产品解说片两个作者案例，链接已有 Hypit Skill。
数据在 `lib/showcase-hypit.ts`；视频用作者发布的外部地址，按点击加载，不托管完整视频。
本地仅保存真实视频帧和压缩预览，出处与权利边界见 `hypit-NOTICES.txt`。
两条案例使用原有筛选、作者、播放器、任务交接、Skill 关联、SEO 与惰性投票登记流程。
软件标为“源码可用”，不把附加商业条款标成无限制开源；也不把软件许可套用到视频素材。
没有执行 Hypit、付费生成或写入安装成功数据。以下 100 个案例为历史首批说明。
...
- `/showcase?category=video`：20 个视频案例统一以封面和“播放预览”按钮展示，点击后原位播放；标题进入案例详情。与详情页共用播放器，点击前不挂载视频源，不批量预载或悬停自动播放，播放新视频时暂停其他视频。
- 视频页底部的“相关视频技能”是默认折叠的辅助推荐，展开后使用同一网格风格；原 `#video-skills` 分享地址仍能打开它。5 个工具与作品分别计数，不能把工具推荐误标成上方案例的作者或实测结果。当前审核状态以技能详情为准。
...
一级分类只表达交付形式，二级用途在与作者、排序并列的下拉框中筛选：Logo 与品牌标识、产品宣传、知识讲解、数据讲述。条件求交集，没有当前匹配的用途选项禁用；已选条件仍可清除。不得把技能目录链接混入用途筛选栏。
