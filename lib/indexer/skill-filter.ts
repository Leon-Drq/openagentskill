...
export function evaluateSkillCandidate(candidate: SkillCandidateProfile): SkillCandidateEvaluation {
  ...
  if (candidate.topics?.some((topic) => /skill|agent|automation|rag|crawler|scraping|browser|document|pdf|ocr|testing|security|semantic-search|vector|finance|quant|trading|backtesting|portfolio|fintech|market-data|risk-management|technical-analysis|options-pricing|research|arxiv|etl|analytics|business-intelligence|ecommerce|sales|devops|kubernetes|terraform|observability|mlops|geospatial|bioinformatics/i.test(topic))) {
    score += 1
  }
