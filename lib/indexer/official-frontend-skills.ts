...
const USER_AGENT = 'OpenAgentSkill-OfficialSkillIndexer/1.0'
...
type OfficialFrontendSkillSlug = (typeof OFFICIAL_FRONTEND_SKILL_SLUGS)[number]
...
export interface OfficialFrontendSkillSyncEntry {
  slug: OfficialFrontendSkillSlug
  status: 'created' | 'updated' | 'error'
  reason?: string
}

export interface OfficialFrontendSkillSyncResult {
  status: 'completed' | 'completed_with_errors'
  verified: number
  created: number
  updated: number
  errors: number
  entries: OfficialFrontendSkillSyncEntry[]
}

function officialFrontendSkills(): SkillRecord[] {
  const wanted = new Set<string>(OFFICIAL_FRONTEND_SKILL_SLUGS)
  const skills = CURATED_SKILL_SNAPSHOT.filter((skill) => wanted.has(skill.slug))

  if (skills.length !== OFFICIAL_FRONTEND_SKILL_SLUGS.length) {
    throw new Error('The official frontend skill snapshot is incomplete.')
  }
...
function sourceReference(skill: SkillRecord): SourceReference {
  ...
  if (url.hostname !== 'github.com' || treeIndex !== 2 || parts.length < 5) {
...
function rawSkillUrl(source: SourceReference) {
  return `https://raw.githubusercontent.com/${source.repository}/${source.branch}/${source.skillDirectory}/SKILL.md`
}

function sourceUrl(source: SourceReference) {
  return `https://github.com/${source.repository}/tree/${source.branch}/${source.skillDirectory}`
}
...
function buildPayload(skill: SkillRecord, source: SourceReference, metadata: GitHubRepositoryMetadata) {
  return {
    ...
    submitted_by_agent: 'official-skill-path-indexer',
    ai_review_score: {
      ...
      skill_path: `${source.skillDirectory}/SKILL.md`,
    },
...
/**
 * Imports explicit SKILL.md paths from maintained upstream repositories.
 * Each path is verified before the guarded database upsert runs, so a broad
 * repository cannot accidentally masquerade as a single installable skill.
 */
export async function syncOfficialFrontendSkills(): Promise<OfficialFrontendSkillSyncResult> {
  const serverSecret = (process.env.INDEXER_SECRET || '').trim()
  if (!serverSecret) {
    throw new Error('Missing INDEXER_SECRET for controlled official skill sync.')
  }
  ...
  const entries: OfficialFrontendSkillSyncEntry[] = []
  let verified = 0
  let created = 0
  let updated = 0

  for (const skill of officialFrontendSkills()) {
    const slug = skill.slug as OfficialFrontendSkillSlug
    ...
    try {
      ...
      if (data?.created) {
        created += 1
        entries.push({ slug, status: 'created' })
      } else {
        updated += 1
        entries.push({ slug, status: 'updated' })
      }
    } catch (error) {
      entries.push({
        slug,
