...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  const description = zh ? `探索 ${SHOWCASE_CASES.length} 个技能作品、作者模板与风格示例，覆盖网页、演示、图像、视频和文档。每个案例包含预览、使用条件与来源。` : locale === 'en' ? `Explore ${SHOWCASE_CASES.length} skill examples, author templates and style studies across web, slides, images, video and documents, with tasks, requirements and source credits.` : galleryCopy(locale, 'Discover the work, meet its creators, and make something of your own.', '')
